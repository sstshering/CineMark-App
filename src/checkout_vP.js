  ...
  document.getElementById("checkout").addEventListener("submit", async function (event) {
    ...
    if (inputYear > currentYear || (inputYear == currentYear && inputMonth >= currentMonth)) {      
      ...
      if (user) {
        ...
        if(window.localStorage.getItem('fromMember') == '1'){  
      ...
      // Redirect the user to the viewProfile.html page after membership is updated
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      if (dropdown) {
        ...
        console.log("Element with id 'saved' not found.");
      }
    } else {
